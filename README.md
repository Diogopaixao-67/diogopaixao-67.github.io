...
<!-- ===== PLAYMATES: BOTÃO DE TEMA (PRETO/BRANCO) ===== -->
...
<!-- OPÇÕES DE TEMA -->
<div id="themeOptions">
  <button onclick="setTheme('light')">Modo Branco</button>
  <button onclick="setTheme('dark')">Modo Preto</button>
</div>

<script>
  // Mostrar / esconder as opções
  ...
  function setTheme(mode) {
    if (mode === "dark") {
      ...
      localStorage.setItem("playmates_theme", "dark");
    } else {
      document.body.classList.remove("dark-mode");
      localStorage.setItem("playmates_theme", "light");
    }
    document.getElementById("themeOptions").style.display = "none";
  }

  // Aplicar tema salvo automaticamente
  window.onload = () => {
    const saved = localStorage.getItem("playmates_theme");
    if (saved === "dark") {
      document.body.classList.add("dark-mode");
    }
  };
