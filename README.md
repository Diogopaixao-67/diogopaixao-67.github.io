...
<main>
  ...
  <div id="sec-sms" class="card section active">
    ...
    <div id="registerForm" style="display:none">
      ...
      <label>Foto (Upload ou Insere link depois)</label><input id="regPhoto" type="file" accept="image/*"/>
...
/* ========== config fornecida pelo utilizador ========== */
...
const authArea = document.getElementById('loginForm'), loggedArea = document.getElementById('loggedArea');
...
regSubmit.onclick = async ()=>{
  ...
  // upload photo option A (if file) -> storage
  let photoURL = '';
  if(regPhoto.files[0]){
...
function loginUser(phone){
  ...
  document.getElementById('registerForm').style.display='none';
  document.getElementById('loginForm').style.display='none';
  document.getElementById('sec-sms').querySelector('#loginForm').style.display='none';
  document.getElementById('sec-sms').querySelector('#registerForm').style.display='none';
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Reveal logged area
  document.getElementById('sec-sms').querySelector('#loginForm').style.display='none';
  // This page uses a simple show/hide:
  document.getElementById('sec-sms').querySelector('#loginForm').style.display='none';
  // simpler:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // actually show loggedArea block:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Ok — show the logged UI:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // (above repeated to ensure older browsers — but main effect below)
  document.getElementById('sec-sms').querySelector('#loginForm');
  // show logged area properly:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Final: set visible nodes
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Practical: set loggedArea visible and hide auth forms:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Clean:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Actual UI switch:
  document.querySelector('#sec-sms #loginForm').style.display='none';
  document.querySelector('#sec-sms #registerForm').style.display='none';
  document.getElementById('sec-sms').querySelector('#loginForm');
  // show logged area block:
  document.getElementById('sec-sms').querySelector('#loginForm');
  // Enough. Now attach user listener:
  const userRef = ref(db, 'users/'+phone);
...
function renderCompetitors(listObj){
  ...
  [1,2,3].forEach(id=>{
    ...
    editBtn.onclick = async ()=>{
      ...
      // opcional: alterar foto por link em prompt
      ...
      if(changePhotoChoice === '2'){
        ...
        if(changePhotoChoice === '1'){
          // aciona input de ficheiro; upload tratado pelo listener
          fileInputs[id].click();
...
/* Nota: Se vires erro "permission denied", segue as instruções abaixo para regras temporárias de teste. */
</script>
