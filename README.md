...
window.apagarEvento=(id)=>{
  ...
  // ---------- Início do bloco JOGOS (colar no <script type="module">) ----------
/* Estrutura de dados no Realtime DB usada:
   /jogos/countdown -> { target: <timestamp ms> }
   /jogos/competitors/<id> -> { name, school, votes, photo }
   /jogos/votes -> registros de votos {voter, competitor, ts}
*/
...
// --- Função utilitária para formatar HH:MM:SS
...
// --- Contador: le o target do DB e atualiza em tempo real
...
// Atualiza a label localmente a cada 1s (para economizar leituras DB)
setInterval(()=>{
  if(currentTarget){
...
editCountdownBtn.onclick = async () => {
  ...
  // Pergunta horas, minutos, segundos
...
// --- Renderiza os 3 concorrentes inicialmente vazios se não existirem
async function ensureDefaultCompetitors(){
  const snapshot = await get(competitorsRef);
  if(!snapshot.exists()){
    const defaults = {
      1: { name: "Concorrente 1", school: "", votes: 0, photo: "" },
      2: { name: "Concorrente 2", school: "", votes: 0, photo: "" },
      3: { name: "Concorrente 3", school: "", votes: 0, photo: "" },
    };
...
function renderCompetitors(listObj){
  ...
  ids.forEach(id=>{
    ...
    editBtn.onclick = async () => {
      ...
      // votos manualmente? permitimos editar
      const newVotes = parseInt(prompt("Número de votos (apenas números):", c.votes||0)) || 0;
      // pedimos ao utilizador para escolher foto via input oculto
      const changePhoto = confirm("Deseja alterar a foto do concorrente?");
      let photoURL = c.photo || "";
      if(changePhoto){
        // disparar file input para este concorrente
        fileInputs[id].click();
        // aguardamos o upload (o listener do input fará o upload e atualizará o DB)
        // mas também aceitamos continuar e atualizar nome/escola/votos agora
      }
      // Atualiza os campos (photo será atualizado pelo upload quando terminar)
      await set(ref(db, `jogos/competitors/${id}`), { name: newName, school: newSchool, votes: newVotes, photo: photoURL });
      alert("Concorrente atualizado. Se alterou foto, aguarde upload concluir.");
    };

    // Votar: abre WhatsApp e incrementa vote no Firebase
    const voteBtn = document.createElement("button");
    voteBtn.innerText = "VOTAR";
    voteBtn.style.background = "#25D366";
    voteBtn.style.color = "#fff";
    voteBtn.onclick = async () => {
      if(!currentUser) return alert("Faça login para votar.");
      // obter dados do votante (nome se disponível)
      ...
      // incrementa o contador de votos com transação para segurança
      runTransaction(ref(db, `jogos/competitors/${id}/votes`), (current) => {
        return (current || 0) + 1;
      });
      // guarda registo de voto
      push(ref(db, "jogos/votes"), { voter: currentUser, competitor: id, ts: Date.now() });
    };

    // botão para editar foto diretamente (abre file input)
    ...
    // Quando o DB mudar, atualizamos a foto e votos (manter referência por id)
    wrapper.dataset.compId = id;
    competitorsList.appendChild(wrapper);

    // listener para atualizações específicas de votes (opcional, mas já há onValue global abaixo)
  });
}

// Listener global para concorrentes
onValue(competitorsRef, snap => {
  const val = snap.val();
  renderCompetitors(val);
  // também atualiza os votos exibidos se existirem
  if(val){
    [1,2,3].forEach(id=>{
...
// --- Upload de fotos: cada input tem listener que envia imagem ao Storage e grava URL no DB
[1,2,3].forEach(id=>{
  ...
  input.onchange = async (e) => {
    ...
    try{
      ...
      await set(ref(db, `jogos/competitors/${id}/photo`), url);
      // atualiza apenas o campo photo sem sobrescrever outros dados
      const compSnap = await get(ref(db, `jogos/competitors/${id}`));
      const comp = compSnap.exists() ? compSnap.val() : {};
      comp.photo = url;
      await set(ref(db, `jogos/competitors/${id}`), comp);
